...
export function CaseModal({ study, onClose, onNavigate }: Props) {
  ...
  // panelRef  — outer panel wrapper: slide / drag transforms, group-hover context
  // scrollRef — inner scrollable div: scroll reset, scrollTop checks, touch & wheel events
  ...
  // Reset scroll position and animate new case in
  useEffect(() => {
  ...
  // Horizontal trackpad / wheel swipe → navigate
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (Math.abs(wheelAccum.current) > 120) {
        ...
        navigateDir(dir);
        window.setTimeout(() => { wheelCooldown.current = false; }, 600);
      }
    };
  ...
  // ── Touch handlers (drag-to-close + horizontal swipe to navigate) ───────────
  ...
  // ── Slide / drag transform (applied to outer panel wrapper) ─────────────────
  const panelTransformStyle = (() => {
    let transform: string | undefined;
    let transition: string | undefined;
    if (dragY > 0) {
      transform  = `translateY(${dragY}px)`;
      transition = touchActive.current ? "none"
        : (closing || dragY === 0) ? "transform 280ms ease-out"
        : undefined;
    } else if (slide) {
  ...
  // ── Shared chevron button classes ────────────────────────────────────────────
  const chevronBase =
    "hidden md:flex absolute top-1/2 -translate-y-1/2 z-20 items-center justify-center " +
    "w-10 h-16 text-white select-none pointer-events-auto " +
      ...
      <div className="absolute inset-0 flex items-end justify-center md:items-stretch md:justify-end">
        ...
        >
          ...
          <div
            ...
            className="w-full h-full overflow-y-auto overscroll-contain text-cream"
          >
            {/* Mobile drag handle */}
            ...
            <figure className="relative w-full">
              {study.video ? (
                <video
                  ...
                  style={{ backgroundColor: "#0A1628", ...(study.slug === "danmarks-radio" ? { filter: "brightness(0.85)" } : {}) }}
                  autoPlay
                  muted
                  playsInline
                  preload="auto"
            ...
            <article key={study.slug} className="px-6 md:px-10 py-10 md:py-14 space-y-10 md:space-y-12">
              ...
              <ModalSection title="Kontekst" rootRef={scrollRef}>
                <p className="text-base md:text-lg text-cream/85 leading-relaxed">{study.context}</p>
              </ModalSection>

              <ModalSection title="Udfordring" rootRef={scrollRef}>
              ...
              <ModalSection title="Min rolle" rootRef={scrollRef}>
              ...
              <ModalSection title="Tilgang" rootRef={scrollRef}>
                <ApproachGrid tags={study.approach} />
              </ModalSection>

              <ModalSection title="Resultater" rootRef={scrollRef}>
                <ul className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-8">
                  ...
                  <div className="md:col-span-9 flex justify-start">
                    <div className="relative w-full max-w-[480px]">
                      <img src={boligaMockup} alt="Boliga app mockups: vælg kommune, drømmebolig, boligtype og præferencer" className="w-full h-auto" />
                      <div aria-hidden className="pointer-events-none absolute inset-x-0 bottom-0 h-24 md:h-32 bg-gradient-to-b from-transparent to-[#0D1B2A]" />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-8">
                  ...
                  <div className="md:col-span-9 flex justify-start">
                    <div className="relative w-full max-w-[384px]">
                      <img src={woltHeatmap} alt="Heatmap der viser efterspørgsel i realtid i København" className="w-full h-auto" />
                    </div>
                  </div>
                </div>
              )}
              ...
              {study.slug === "amnesty-international" && (
                <div className="mt-12 flex flex-col" style={{ gap: "4px" }}>
                  <div className="overflow-hidden aspect-[21/9]">
                    <img src={amnestyStudio} alt="Lydstudie med person i optagerum" loading="lazy" className="w-full h-full object-cover grayscale transition-transform duration-300 ease-in-out hover:scale-105" />
                  </div>
                  <div className="grid grid-cols-2" style={{ gap: "4px" }}>
                    {[
                      ...
                      <div key={i} className="overflow-hidden aspect-[4/3]">
                        <img src={img.src} alt={img.alt} loading="lazy" className="w-full h-full object-cover grayscale transition-transform duration-300 ease-in-out hover:scale-105" />
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {study.gallery && study.gallery.length > 0 && (
                <div className="space-y-4 md:space-y-6">
                  <img src={study.gallery[0]} alt={`${study.client} — billede 2`} loading="lazy" className="w-full h-auto rounded-md" />
                  {study.gallery.length > 1 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                      {study.gallery.slice(1).map((src, i) => (
                        <img key={i} src={src} alt={`${study.client} — billede ${i + 3}`} loading="lazy" className="w-full h-auto rounded-md object-cover" />
                      ))}
                    </div>
                  )}
                </div>
          ...
          {/* ── end inner scrollable div ─────────────────────────────────── */}

          {/* ── Left chevron ────────────────────────────────────────────────
              Appears on panel hover. Hidden on the first case. */}
          {!isFirst && (
            <button
              type="button"
              onClick={() => navigateDir(-1)}
              aria-label="Forrige case"
              className={`${chevronBase} left-3`}
            >
              <span aria-hidden className="text-[2rem] leading-none drop-shadow-[0_1px_4px_rgba(0,0,0,0.6)]">‹</span>
            </button>
          )}
          ...
          {nextCase && (
            <div
              className="hidden md:block absolute right-0 top-0 bottom-0 w-[50px] z-10 cursor-pointer overflow-hidden"
              onClick={() => navigateDir(1)}
